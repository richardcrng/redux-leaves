...
export const combineReducerLeaves = (dict = {}, { prefix = "", route = [] } = {}, augment = true) => {
  const reducer = combineReducers(recursivelyCombinedDict(dict, prefix, route))
...
const recursivelyCombinedDict = (dict = {}, prefix, route) => {
  return _.mapValues(dict, (val, key) => {
    if (typeof val === "function") {
      return val
    } else if (typeof val === "object") {
      return combineReducerLeaves(val, prefix, [...route, key])
    } else {
      return val
    }
  })
